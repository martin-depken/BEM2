...
def sim_anneal_fit(xdata, ydata, yerr, Xstart, lwrbnd, upbnd, model,
    ...
    InitialLoop(SA, X, xdata, ydata, yerr, lwrbnd, upbnd, OutputFitResults)
...
def chi_squared(xdata, ydata, yerr, params, model):
    ...
    # Calculate residuals
    model_result = model(np.array(xdata),np.array(params))
    residual = ( (model_result-ydata)/yerr )**2
    return np.sum(residual)

def V(SA, xdata,ydata,yerr,params):

    residual_sum = SA.model(xdata,ydata,yerr,params,SA)
...
class SimAnneal():
    ...
    def __init__(self, model, Tstart, delta, tol, Tfinal,adjust_factor, cooling_rate, N_int,
        ...
        if self.MP:
            self.processes = [mp.Process(target=worker_func,args=(self.inQ,self.outQ)) for i in range(self.nprocs)]
...
def InitialLoop(SA, X, xdata, ydata, yerr, lwrbnd, upbnd,OutputFitResultsFile):
    '''
    ...
    while True:
        ...
        if (steps % SA.interval == 0):
            for parameter in X:
                OutputFitResultsFile.write(str(parameter) + '\t')
...
def worker_func(inQ,outQ):
    while True:
        try:
            ...
            residual = chi_squared(xdata,ydata,yerr,params,model)
